...
export class OrderViewComponent {
  ...
  async ngOnInit() {
    ...
    // 2. Verificamos si es CLIENTE (Rol 3)
    if (user && user.rol === 3) {
      this.isClient = true;
  ...
  totalOrder() {
    this._order.formOrder.controls['total'].patchValue(
      this.totalProducts() + this.totalExtras()
    );
  ...
  async placeOrder() {
    ...
    if (this._order.formOrder.valid && this._wsService.socketStatus) {
      ...
      if (data) {
        ...
        this._snackBar.open('Orden realizada', '', {
          duration: 3000,
        ...
        this._snackBar.open('No se realizó la orden', '', {
          duration: 3000,
          verticalPosition: 'top',
        });
      }
    } else {
      this._snackBar.open('No es posible realizar la orden', '', {
        duration: 3000,
        verticalPosition: 'top',
      });

      document
        .querySelectorAll('.ng-invalid, .mat-mdc-radio-group.unselect')
